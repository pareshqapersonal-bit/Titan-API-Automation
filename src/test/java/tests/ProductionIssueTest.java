...
public class ProductionIssueTest extends BaseTest {
	...
	for (int i = 1; i <= 50; i++) {
			...
			RemoveCartItemAPI rcapi = new RemoveCartItemAPI();
			CartTotalPayload cpayload = new CartTotalPayload();
			cpayload.setCartId(quoteId);
		
			response =rcapi.cartTotal(mobileSpec, cpayload, customerToken);
			
			int cartTotal= Integer.parseInt(response.jsonPath().getString("res.grand_total"));
//			Assert.assertEquals(itemCount, 1);
			Assert.assertTrue(cartTotal > 0);
			...
			RemoveCartItemAPI rca = new  RemoveCartItemAPI();
					...
					response =rcapi.cartTotal(mobileSpec, cpayload, customerToken);
					 ...
					 cartTotal= Integer.parseInt(response.jsonPath().getString("res.grand_total"));
					Assert.assertEquals(cartTotal, 0.0);
